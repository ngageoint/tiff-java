	...
	public void testWriteCustom() throws IOException {
		...
		for (int y = 0; y < inpHeight; y++) {
			for (int x = 0; x < inpWidth; x++) {
				inpPixVals[y * inpWidth + x] = (int) (Math.random() * Math.pow(
						2.0, bitsPerSample));
			}
		}
		...
		TestCase.assertEquals(TiffConstants.RESOLUTION_UNIT_INCH, fileDirectory
				.getResolutionUnit().intValue());
		...
		TestCase.assertEquals(TiffConstants.COMPRESSION_NO, fileDirectory
				.getCompression().intValue());
